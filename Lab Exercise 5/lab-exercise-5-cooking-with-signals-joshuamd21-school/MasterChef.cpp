...
// Removes the completed steps from the dependency list of the step list.
...
int main(int argc, char **argv)
{
	...
	while (recipeSteps->Count() > completeCount)
	{
		...
		for (long unsigned int i = 0; i < ready.size(); i++)
		{
			makeTimer(ready.at(i), ready.at(i)->duration);
			ready.at(i)->running = true;
		}
		sleep(1);
	}
